// Pure policy for client work requests: rate limits, question budget,
// and the untrusted-text wrapper. No DB, no AI — so all of it is tested.
...
export function suggestEstimate(
  ...
  return {
    ...
    basis: related.length >= 2
      ? `${related.length} milte-julte tasks ka median`
      : `pichle ${pool.length} tasks ka median`,
  };
}
